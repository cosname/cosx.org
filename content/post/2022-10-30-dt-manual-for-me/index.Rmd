...
属性：元素有元素属性，通常是各元素提供的附加信息，如对表格某一列设置宽度属性，将会改变表格中目标列的宽度；样式有样式属性，如字体样式包括字体颜色、字体大小、字体粗细等。

参数/参数值：R 函数中能够输入可选内容的变量。每个参数能够输入的具体值。比如对表格某一列设置宽度为'300px'时，对应的参数和参数值就是`width = '300px'`。
...
回调函数的作用是在 R 中引入 JS，引入 JS 就可以引入 html 和 css，主要方法是使用来自于 htmlwidgets 包的`JS()`函数。回调函数有好几种，所需输入参数不同，起到的作用也不同。
...
还有针对表头的回调函数`headerCallback`，针对每次分页绘图渲染的回调函数`drawCallback`等等。值得一提的是，既然都可以写入  JS，那么事实上调用不同的回调函数也可以达到相同的效果。比如在后文1.5.1小节中，可以通过调用初始化回调函数来修改表头的 css 样式，也可以直接调用表头的回调函数来实现。
...
一般情况下表格的高度和宽度是与显示页面的大小自适应的，也可以单独设置整个表格的高度和宽度。
...
对应修改行名称的参数是`rownames`，可填入的具体参数值有 TRUE/FALSE，或者字符向量。
...
对应修改列名称的参数是`rownames`，可以指定部分列名进行修改，也可以一次修改全部列名称。
...
在 DT 包中若要给表格添加更多组成部分，如多行表头、表格标题、表格脚注等，需要引入 htmltools，相当于在 R 中引入 html。DT 包中引入 htmltools 的方式有两种：其一，从 DT 包已有参数中引入，比如`caption`和`container`；其二，把整个表格引入一个纯 html 的 div 块中。
...
表格容器里还可以直接添加表格标题和脚注，并且可以分别为标题和脚注引入预先定义好的 css 样式。
...
前面两个小结是在表格中引入 htmltools，从而丰富表格的组成部分，也可以换个思路，把表格引入到 htmltools 中，同样也可以引入预先设定好的 css 样式。使用此方法须对 html 元素有一定了解，比如多个 div 框套在一起时，父 div 框和子 div 框之间的元素属性继承问题。
...
DataTables 中单元格样式的类别如下，可以分别对表格的表头（head）和表格主体（body）设置文本对齐样式。在 DT 中也可以做到。
...
datatable(data,
          options = list(columnDefs = list(
            # 表头居右
            list(className = 'dt-head-right', targets = 5), 
            # 表格主体居左
            list(className = 'dt-body-left', targets = 5), 
            # 表格的表头和主体均居中
            list(className = 'dt-center', targets = c(6, 7)) 
          )))
```
...
DT 包中封装好的`formatStyle()`函数可直接为目标列引入 css 样式，该函数除了可以为每一列设定样式，还可以为每一列中不同行设定不同的样式。其中可引入的 css 属性有很多，比如单元格背景颜色（backgroundColor）、边框（border）、 字体大小（font-size）、文本对齐（text-align）等等，基本语法如下。
...
所有的 css 属性参数可以填写具体的 css 属性参数值，比如设定`color= 'red'`，那么目标列的字体颜色就会变成红色。还可以引入以下函数，为目标列中不同行设定不同的参数值。
...
以下是将多种方法集于一体的复杂例子，为指定的某一列或某几列设置背景颜色，为不同行设定不同的背景颜色，包括[填充渐变颜色](https://yuanfan.rbind.io/project/dt-fill-color/)。
...
使用`styleColorBar()`函数为单元格填充带有颜色的条形，这是专门为单元格背景样式开发的函数，不能挪用到其他样式上去。
...
也可以给目标列设置各种不同的 css 字体样式，如字体粗细、字体颜色、字体大小等。
...
也可以为单元格设置文本对齐方式。需要注意的是，`formatStyle()`函数中设定的各类 css 样式仅仅会修改表格主体的样式，不会改变表头的样式。
...
DataTables 中的 [API](https://datatables.net/reference/api/) 功能非常丰富，本小节只简单介绍其中用于设置表格样式的部分。方法是使用 DT 包中的初始化回调函数`initComplete = JS()`来调用 DataTables API，再引入 css 来设置表格各个组成部分的样式，以下是几个简单例子。
...
可以通过列渲染的方式来设置单列的样式。需要注意的是，此方法的本质是通过`JS()`函数引入 html 元素，再引入 css 来设定样式，具体呈现效果与元素本身的特性息息相关。如下面例子中使用`<div>`元素设定边框样式，本质是在表格的单元格中把原来的数据用个框框起来，一般情况下边框不会把表格沾满，设置的左边框线也不会是连续的。而`<span>`元素本身是用来组合文本的，在这种情况下设定边框样式，边框线会紧紧贴着文本。
...
+ `formatString(table, columns, prefix = "", suffix = "", rows = NULL)`。
    ...
    - prefix/suffix：填入放在一列数据值之前、之后的字符串，同`formatCurrency()`函数中的 currency 参数一样，除了可以填入普通的字符串，还可以填入 UNICODE 编码来展示特殊字符。
...
+ `formatPercentage()`：将表格中的目标列的数字格式化为百分比。
...
+ `formatRound()`：将表格中的目标列的数字四舍五入到指定的小数位数。
...
DT 中用于转换各类日期格式的函数是`formatDate()`，可选参数如下。

+ columns：用于指定转换日期格式的目标列，可以为一列或多列。
+ method：可选的转换日期格式的方法，见`DT:::DateMethods`，共有"toDateString"、"toISOString"、"toLocaleDateString"、"toLocaleString"、"toLocaleTimeString"、"toString"、"toTimeString"、"toUTCString"等8种方法，设定具体参数值可填入一种或多种。
...
HTML 中的`<a>`标签的 [href 属性](https://www.w3school.com.cn/tags/att_a_href.asp)可写入所需指向的超链接地址。链接的样式可以更改，对应的 HTML 代码为`<a class="自定义 css 样式" href="超链接地址" title="超链接title">超链接显示名</a>`。在 DT 包中的实现方法是，针对目标列以`render = JS()`的形式引入 JS，在此基础上引入 HTML 和 css。
...
HTML 中的[`<img>`元素](https://www.w3school.com.cn/tags/tag_img.asp)可写入所需展示的图像地址，对应的 HTML 代码为`<img class="自定义 css 样式" src="图片链接" alt="图片alt">`，图片的 alt 属性是指当图像无法正常显示时，页面上展示的替代文本。这里的图片链接可以是指向图片的网页链接，也可以是本地文件中的图片地址。
...
shiny 包中有个函数`icon()`可以方便地从字体图标库[Font Awesome](https://fontawesome.com/icons)或[Bootstrap Glyphicons](https://getbootstrap.com/docs/3.3/components/)引入小图标。而 Font Awesome 库在 R 中也有对应的 R 包 [fontawesome](https://cran.r-project.org/web//packages/fontawesome/fontawesome.pdf)，本小节根据该库举两个简单的例子。
...
由于 DT 包的渲染方式是一次只渲染一页数据，在有多页数据需要渲染迷你图时，需要在`options`中设定`drawCallback = JS('function(s) { HTMLWidgets.staticRender(); }') `。
...
DT 包中可设置静态样式的方法较多，最好的情况是一次只用一种方法设置好所需全部样式，否则用不同的方法设置的样式会互相冲突。
...
在筛选框输入过滤条件进行筛选时，一般筛选范围是整张表的所有内容。在数据量较大时，全表筛选的效率可能不高，于是也可以专门设置仅针对单列数据进行筛选的筛选框。
...
编辑表格内容的参数是`editable`，可填入的参数值有 TRUE/FALSE、row、column、cell、all 中任一种，或者一个特别设定的列表。需要注意的是，不能编辑表头。
...
都支持 crosstalk 的交互式绘图包和交互式表格包一起使用时，可以通过 crosstalk 实现图表数据共享，比如 plotly 和 DT。
