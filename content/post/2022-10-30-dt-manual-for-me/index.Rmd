...
这是一份针对 R 中的表格包DT 的速查手册。DT 包源自于 JavaScripts（以下简称 JS） 中的 DataTables，但并非所有 DataTables 提供的功能都能在 DT 中实现，而 DT 中也有独立于 DataTables 之外的一些更贴合 R 语法的函数。
...
dt <- as.data.table(data) #后文中用于数据处理
...
由于 DT 包并不是一个完全独立且封闭的表格系统，且本身作为一个以动态交互功能为主的表格包子，在设置静态样式方面往往需要引入 html、css。为了不使描述几种语言的特性时各种术语互相混淆影响阅读，笔者特别对以下术语的含义做出如下约定。

元素：源于 html 元素，表格元素是构成表格的基本组成部分，如表格的表头、表格主体、标题、脚注、行、列、单元格等。

样式：源于 css 样式，设置样式可对各表格元素进行修饰，如表格的边框样式、表格所展示数据的字体样式等。

属性：元素有元素属性，通常是各元素提供的附加信息，如对表格的表头设置宽度属性，将会改变表格的表头单元格的宽度；样式有样式属性，如字体样式包括字体颜色、字体大小、字体粗细等。

参数：R 函数中可输入可选内容的变量。

### 1.0.2.参数位置

DT 中可设置的参数非常多，可以同时为多个表设置共用的全局参数，也可以为单个表设置单独的可选参数，或进一步为单个表中的一列或多列设置单独的可选参数。
...
回调函数的作用是在 R 中引入 JS，需要使用来自于 htmlwidgets 包的`JS()`函数。回调函数有好几种，所需输入参数不同，起到的作用也不同。
...
还有针对表头的回调函数`headerCallback`，针对每次分页绘图渲染的回调函数`drawCallback`等等。值得一提的是，既然都可以写入  JS，那么事实上调用不同的回调函数也可以达到相同的效果。比如在1.5.1小节中，可以通过调用初始化回调函数来修改表头的 css 样式，也可以直接调用表头的回调函数来实现。
...
一般情况下表格的高度和宽度是与显示页面的大小自适应的，也可以单独设置整个表格的高度和宽度,如下。

```{r,eval=FALSE}
datatable(data, height = 250, width = 600)
```

或者，也可以单独设置某一列或者某几列的宽度。
...
表格中对应修改行名称的参数是`rownames`，可填入的具体参数值有 TRUE/FALSE，或者字符向量。
...
+ 自行指定行名称时，需要填入一个字符向量，这个向量的长度应该跟表格的行数保持一致。
...
表格中对应修改列名称的参数是`rownames`，可以指定部分列名进行修改，也可以一次修改全部列名称。
...
+ 若仅替换部分列名称，可如此指定需要替换列名称的列，以及新的名称。当用列的序号指代列，最好隐藏行名称。
...
在 DT 包中若要给表格添加更多组成部分如多行表头、表格标题、表格脚注等，需要引入 htmltools，相当于在 R 中引入 html。DT 包中引入 htmltools 的方式有两种：其一，从 DT 包已有参数中引入，比如`caption`和`container`；其二，把整个表格引入一个纯 html 的 div 块中。
...
若是引入 htmltools 中的函数，那么可以单独设置表格标题的 css 样式。如下，`caption-side: top`表示表格标题的位置居于表格上方，`text-align: left`表示文本对齐方式为居左，`font-size: 20px`表示文本中字体大小为“20px”，`font-weight: bold`表示文本中字体粗细为加粗。
...
当然，htmltools 是可以一次引入多个的。如下，在`caption`参数中可以引入多级标题，并且各自设定单独的 css 样式。
...
也可以尝试通过 htmltools 引入其他 html 元素，如下。
...
|:---------:|:-------------:|:---------------:|
|`<table>`||定义表格|
|`<th>`|table header cell|定义表格表头的单元格|
|`<tr>`|table row|定义表格的行|
...
如下，`thead()`即 html 中的`<thead> </thead>`，表示表格的表头；`tr()`即 html 中的`<tr> </tr>`，表示表格的行；`th()`即 html 中的`<th> </th>`，表示表格表头中的单元格。那么，`tr(th())`为表格上一行表头中的一个单元格。需要多少行表头则需要写多少个`tr()`，一行表头中有多少单元格则需要写多少个`tr(th())`。其中，`rowspan`用于设置单元格可横跨的行数，`colspan`用于设置单元格可横跨的列数。
...
前面两个小结是在表格中引入 htmltools，从而丰富表格的组成部分，也可以换个思路，把表格引入到 htmltools 中。使用此方法须对 html 元素有一定了解，比如多个 div 框套在一起时，父 div 框和子 div 框之间的元素属性继承问题。
...
DT 包中有一些现成的参数可用来设置表格样式。

### 1.3.1.表格主体的默认样式

DT 包继承了 DataTables 的[默认样式](https://datatables.net/manual/styling/classes#Table-classes)，可以使用以下类名的任意组合来构建所需的表样式。虽然本章节的章节名是“静态样式”，但由于 DataTables 本身是用于展现动态表格的，因此默认样式实际上是各种动态样式。
...
DataTables 中单元格样式的类别如下，可以分别对表格的表头（head）和表格主体（body）设置文本对齐样式。
...
datatable(data,
          options = list(columnDefs = list(
            list(className = 'dt-head-right', targets = 5), # 表头居右
            list(className = 'dt-body-left', targets = 5), # 表格主体居左
            list(className = 'dt-center', targets = c(6, 7)) # 表格的表头和主体均居中
          )))
```

### 1.3.3.自定义 css 样式

除了可以使用默认样式以外，还可以设置自定义样式。方法是先定义好 css 样式名称和具体样式属性，然后用`calss = '样式名称'`的方式引入样式属性。需要注意的是，这种方法是对目标列设定样式，会同时改变该列的表头和表格主体的样式。
...
datatable(data, options = list(columnDefs = list(
  ...
  list(targets = 4, class = 'width')
)))
```

## 1.4.通过`formatStyle()`设置样式

DT 包中封装好的`formatStyle()`函数可引入 css 样式应用于表格，该函数除了可以为每一列直接设定 css 样式，还可以为每一列中不同行设定不同的 css 样式。基本语法如下，可引入的css属性还有很多，比如边框（'border'）、 字体大小（'font-size'）、文本对齐（'text-align'）等等。
...
datatable(data) |>
  formatStyle(
    ...
    color = NULL, # css 属性，字体颜色
    backgroud = NULL, # css 属性，背景填充
    backgroundColor = NULL, # css 属性，背景颜色
    fontWeight = NULL, # css 属性，字体粗细
    …
  )
```

所有的 css 属性参数除了可以目标列指定具体参数值，还可以填以下函数，使得目标列中不同行有不同的参数值。
...
+ `styleColorBar(data, color, angle = 90)`：为单元格填充带有颜色的条形图，条形的宽度与单元格中数据值大大小成正比，其中'angle'参数值用于设定旋转角度。
+ `styleRow(rows, values, default = NULL)`：为目标行设定 css 属性。

### 1.4.1.单元格背景颜色（backgroundColor）

为指定的某一列或某几列设置背景颜色，为不同行设定不同的背景颜色，包括[填充渐变颜色](https://yuanfan.rbind.io/project/dt-fill-color/)。
...
datatable(data) |>
  ...
  # 为第2列，即'type2'列中不同的数据值设定不同的背景颜色
  formatStyle(columns = 2,
              backgroundColor = styleEqual(
                levels = c('YES', 'NO'),
                values = c('IndianRed', 'Firebrick')
              )) |>
  # 为第3列，即'value'列，按照第2列（'type2'）的数据值来设定不同的背景颜色
  formatStyle(
    columns = 3,
    valueColumns = 2,
    backgroundColor = styleEqual(
  ...
  # 为第4列，即'value1'列，按照该列的数据值设定渐变颜色
  formatStyle(columns = 4,
              backgroundColor = styleInterval(cuts = brks, values = clrs)) |>
  # 目标为每行，按照'prob1'列的数据值分段，不断分段的取值范围内设定不同的背景颜色
  formatStyle(
    columns = 'prob1',
    target = 'row',
    backgroundColor = styleInterval(
      cuts = c(0.5, 0.99),
      values = c('gray', 'white', 'pink')
    )
  )
```
...
使用`styleColorBar()`函数为单元格填充带有颜色的条形。
...
datatable(data) |>
  ...
  formatStyle(
    columns = 'value',
    background = styleColorBar(data$value, 'steelblue'), # 填充条形
    backgroundSize = '100% 90%', # 填充背景的尺寸，第一个值为宽度，第二个值为高度
  ...
  formatStyle(
    columns = 'value1',
    background = styleColorBar(data$value1, color = '#00bfff'),
...
datatable(data) |>
  ...
  # 目标为行，按照第2列（'type2'列）的数据值，设定不同行应用不同的字体大小
  formatStyle(
    columns = 2,
    target = 'row',
    fontSize = styleEqual(
...
css 的[边框](https://www.w3school.com.cn/cssref/pr_border.asp)包含三种具体属性，即边框的宽度、边框的线型、边框的颜色，且单元格的上下左右四条边框均可单独设置。

+ 'border'：此参数中可依次填入三种属性值
...
```{r}
datatable(data) |>
  # 为第1列，即'type1'列，设定单元格的边框属性为宽度1px、虚线、黑色
  formatStyle(columns = 1, 'border' = '1px dashed black') |>
  # 为第3列，即'value'列，设定单元格的下边框属性为宽度1px、实线、黑色
  formatStyle(columns = 3, 'border-bottom' = '1px solid black') |>
  # 为第4列，即'value1'列，设定单元格的上边框属性为宽度1px、点线、黑色
  formatStyle(columns = 4, 'border-top' = '1px dotted black') |>
  # 为第5列，即'value2'列，设定单元格的左边框属性为宽度2px、实线、红色
...
### 1.4.7.文本对齐（textAlign）

```{r}
datatable(data)|>
  # 为第1列，即'type1'列设定文本对齐方式为居中
...
DT 包中有一些参数可引入 JS，本小节仅介绍通过引入 JS 来引入 html 元素，在此基础上设置元素属性从而达到设置表格样式的目的。

### 1.5.1.调用 DataTables API 设置样式

DataTables 中的 [API](https://datatables.net/reference/api/) 功能非常丰富，本小节只简单介绍其中用于设置表格样式的部分。方法是使用 DT 包中的`initComplete = JS()`参数调用 DataTables API，引入纯纯的 css 来设置表格各个组成部分的样式。
...
datatable(data,
    ...
    $(this.api().table().header()).css({'color': 'green'});
    }")))
```
...
datatable(data, options = list(
  initComplete = JS(
    "function(settings, json) {
    $(this.api().tables().body()).css({'color': 'red'});
    }")
))
```
...
datatable(data,
          options = list(
            initComplete = JS(
              "function(settings, json) {
    $(this.api().column(3).header()).css({'color': 'green'});
    }")))
```
...
datatable(data,
          options = list(
            initComplete = JS(
              "function(settings, json) {
    $(this.api().column(3).nodes()).css({'color': 'red'});
    }")))
```

### 1.5.2.列渲染（columnDefs.render）

在第1.3.3小节中定义的 css 样式也可以通过列渲染的方式来实现。需要注意的是，此方法的本质是通过`JS()`函数引入 html 元素，再引入元素属性来设定样式，具体呈现效果与元素本身的特性息息相关。如下面例子中使用`<div>`元素设定边框属性，本质是在表格的单元格中把原来的数据用个框框起来，一般情况下边框不会把表格沾满，设置的左边框线也不会是连续的。而`<span>`元素本身是用来组合文本的，在这种情况下设定边框属性，边框线会紧紧贴着文本。
...
datatable(data,
          options = list(columnDefs = list(
            ...
            list(
              ...
              render = JS(
                "function(data, type, row, meta){
              return '<div class=\"width\">' +data+ '</div>'
              }"
              )
            ),
            list(
...
+ `formatCurrency()`：为表格中的数字列添加货币符号（UNICODE编码对应的符号），或者字符串
    ...
    - currency：填入适用于 JS 的[UNICODE编码](https://www.cnblogs.com/lsgxeva/p/10120275.html)，或者一个普通的字符串。如填入'\U20AC'，显示为€；填入'\u21AC'，显示为↬；填入'\u2714'，显示为✔。
...
datatable(data) |>
  formatCurrency(
    ...
    currency = '\u2714',
    before = FALSE, # 指定货币单元是否放在数字前面，默认为TRUE
    digits = 0, # 指定该列数字的小数点后位数，默认保留2位
    rows = c(1:2) # 指定目标列格式化的行
  ) |>
  formatCurrency(
    columns = 6,
    currency = '%',
...
+ `formatString(table, columns, prefix = "", suffix = "", rows = NULL)`
    ...
    - prefix/suffix：填入放在一列数据值之前、之后的字符串，同formatCurrency()函数中的currency参数一样，除了可以填入普通的字符串，还可以填入UNICODE编码来展示特殊字符。
...
+ `formatPercentage()`：将表格中的数字列格式化为百分比
...
+ `formatRound()`：将表格中的数字列四舍五入到指定的小数位数
...
DT中用于转换各类日期格式的函数是`formatDate()`，可选参数如下。

+ columns：用于指定转换日期格式的目标列，可以为一列或多列。
+ method：可选的转换日期格式的方法，见`DT:::DateMethods`，共有"toDateString"、"toISOString"、"toLocaleDateString"、"toLocaleString"、"toLocaleTimeString"、"toString"、"toTimeString"、"toUTCString"等8种方法，具体参数值可填入一种或多种。
...
HTML 中的`<a>`标签的 [href 属性](https://www.w3school.com.cn/tags/att_a_href.asp)可写入所需指向的超链接地址。链接的样式可以更改，对应的 HTML 代码为`<a class="自定义 css 样式" href="超链接地址" title="超链接title">超链接显示名</a>`。在 DT 包中的实现方式是针对目标列以`render = JS()`的形式引入 JS，在此基础上引入 HTML 和 css。
...
HTML 中的[`<img>`元素](https://www.w3school.com.cn/tags/tag_img.asp)可写入所需展示的图像地址，对应的HTML代码为`<img class="自定义 css 样式" src="图片链接" alt="图片alt">`，图片的alt属性是指当图像无法正常显示时，页面上展示的替代文本。这里的图片链接可以是指向图片的网页链接，也可以是本地文件中的图片地址。

在下面的例子中，笔者在项目文件中准备了两张图片，相对地址分别是`images/China.pmg`和`images/USA.png`。
...
### 1.6.6.插入小图标（fontawesome）

shiny 包中有个函数`icon()`可以方便地从字体图标库[Font Awesome](https://fontawesome.com/icons)或[Bootstrap Glyphicons](https://getbootstrap.com/docs/3.3/components/)引入小图标。而 Font Awesome 库在 R 中也有对应的 R 包 [fontawesome](https://cran.r-project.org/web//packages/fontawesome/fontawesome.pdf)。
...
```{r}
icon2 <- dt[, ':='(score = sample(1:5, 40, replace = TRUE))]
...
DT 默认展示文本时自动换行，可以通过单独设置列的宽度或文本对齐属性来改变换行效果。下面编造了一个由长文本组成的数据框，涵盖了三种情况：长文本，列的单元格内容比列名更长，列的列名比单元格内容更长。
...
+ 设置列的文本对齐属性。需要注意的是分别对同一列设置表头和表格主体的文本对齐方式式，需要将对表格主体的设定写在前面，否则此设定会无法生效。

```{r}
datatable(textdata,
          options = list(columnDefs = list(
            list(className = 'dt-nowrap', targets = c(1, 3)),
            list(className = 'dt-body-center', targets = 4),
            list(className = 'dt-head-nowrap', targets = 4)
          )))
```
...
sparkline 是 [jQuery Sparklines](https://omnipotent.net/jquery.sparkline/#s-about) 的 R 版本，所引入的各类图形还有许多参数可以自行设定。由于笔者对 data.table 相对更熟一些，本小节与数据处理相关的部分均应用 data.table 的语法来完成。这里有[一篇笔记](https://yuanfan.rbind.io/project/r-sparkline/)记录了在 DT、reactable、formattable等表格包子中引入 sparkline 的 data.table 版本和 tidyverse 版本的代码。
...
在表格中一列引入迷你图时，用一个`sparkline()`函数，在一列中引入多组迷你图时，需要在`spk_composite()`函数中写入多个`sparkline()`来实现。
...
DT 包中可设置静态样式的方法较多，最好的情况是一次只用一种方法设置好所需全部样式的属性，否则用不同的方法设置的样式会互相冲突。

|序号|方法|所属章节|描述|写入语言|
|:--:|:---------:|:-----:|:-----------------------:|
|1|htmltools|第1.2节|在构建表格元素时设置样式|html/css|
|2|class/className|第1.3节|先定义好 css 样式和样式名，通过`class='样式名'`引入样式，可对全表也可单独对表格的列进行设置|css|
|3|`formatStyle()`|第1.4节|在`formatStyle()`函数中直接写入需要设置的各样式属性和具体属性值，可对表格的列或行进行设置|css|
|4|`JS()`|第1.5节|通过调用 DataTables API对表格表头或表格主体设置样式，或通过列渲染或行渲染对表格的行或列设置样式|JS/html/css|
...
显然，`initComplete = JS()`设定的样式会被`columnDefs = list()`所覆盖。在后者之中，`list(targets = 2, class = 'class1')`会将该列中的表头和表格主体的字体颜色一并修改，而`list(targets = 3, render = JS())`仅修改了该列表格主体的字体颜色。
...
+ 在`formatStyle()`函数中设定第2、3列的字体颜色为蓝色。
+ 在`formatStyle()`函数中设定第4列的字体不同条件不同颜色。

显然，`formatStyle(columns = 2, color = 'blue'`与`list(targets = 2, class = 'class1')`产生了冲突，前者仅能修改该列表格主体的字体颜色，而未能修改表格表头的颜色。而`formatStyle()`中对第3、4列的颜色设定都未能覆盖`list(targets = c(3, 4), render = JS())`。但这并不表明`formatStyle()`中一切样式设置的优先级均低于`render = JS()`。
...
以上只是略作试验和说明，实际应用中碰到的样式冲突问题更为复杂。
...
默认情况下，以上字母代表的表格控件全都展示出来，相当于`dom = 'lftipr'`，在设置`dom`参数时去掉某个字母便去掉了对应的表格控件，改变字母的顺序便改变了对应的表格控件出现的位置和顺序。
...
其中`info`参数中有一些可随表格变动而动态更新的标记，如`_START_`（当前页第一条记录），`_END`（当前页最后一条记录），`_TOTAL_`（筛选后表中总记录数），`_MAX_`（表中总记录数），`_PAGE_`（当前页），`_PAGES_`（表中总页数）。
...
除了可以逐项细节单独修改以外，也可以使用官方提供的插件直接替换。
...
在表格控件中的筛选框输入过滤条件进行筛选时，一般筛选范围是整张表的所有内容。在数据量较大时，全表筛选的效率可能不高，于是也可以专门设置仅针对单列数据进行筛选的筛选框。
...
DataTables 内置的筛选过滤功能会按照输入内容与表格中任何位置的内容进行匹配，但并不仅仅只是简单的字符串对比或匹配，而是允许输入多个字符串来匹配，也可以输入一些正则表达式。
...
+ 使用正则表达式。`regex`参数可用来设定是否允许在筛选时使用正则表达时，DataTables中默认此参数值为`TRUE`，而DT包中则默认此参数值为`FALSE`，即筛选框默认无法使用正则表达式。
...
+ 启用过滤条件。`return`参数默认为FALSE，即当筛选框中输入内容后，对整个表格的过滤功能便实时启用。当设置为`return = TRUE`时，那么必须在筛选框中按下 Enter 键后，过滤功能才会启用。此项参数可能在应用大型数据集时非常有用。
...
选项中的'order'参数可用来设置初始化的排序方式。在使用表格的过程中，初始化设置过的目标列的排序方式仍然可以更改。
...
当指定排序的列较多时，也可以指定各列的排序优先级。通常在`orderFixed = list()`中写在前面的列排序优先级更高，但也可以用'pre/post'参数来设置。如下'pre'表示排序优先级更高，'post'表示排序优先级更低。
...
在上一章中的第1.6.4小节中已经用过隐藏列的功能。本小节介绍将表格中多列的单元格内容合并，并隐藏被合并的一列。如下对列的渲染功能中引入 JS， `row[1]`表示取此表格中第1列数据的值，`row[2]`表示取此表格中第2列数据的值，将两列数据一起返回，合并后的数据便会展示在目标列的单元格中。
...
表格中对应编辑表格内容的参数是`editable`，可填入的参数值有TRUE/FALSE，`'row'`、`'column'`、`'cell'`、`'all'`中任一种，或者一个特别设定的列表。

+ `editable = TRUE`时，等同于`editable = 'cell'`,即在展示的表格中双击选中某个单元格时，可以直接修改该单元格中的内容。若设定`editable = 'row'`，可以选中表格中的一行数据，可包含行名称，不包括表头。若设定`editable = 'column'`，可以表格中的一列数据，不包括表头。若设定`editable = 'all'`，可以选中整个表格，不包括表头。
+ `editable = FALSE`时，不能编辑表格的内容。

```{r}
# 设置第1/3列的内容不能修改
# datatable(data, editable = list(target = 'cell', disable = list(columns = c(1, 3))))
# 设置第1/3列的内容在修改时，仅可输入数字
# datatable(data, editable = list(target = 'cell', numeric = c(1, 3)))
datatable(data, 
          editable = list(
            target = 'cell',
...
DataTables 提供很多[扩展功能](https://datatables.net/extensions/index)，DT 官网中已对[这些扩展功能](https://rstudio.github.io/DT/extensions.html)作了简要介绍，它们的名称分别是 AutoFill、Buttons、ColReorder、FixedColumns、FixedHeader、KeyTable、Responsive、RowGroup、RowReorder、Scroller、SearchPanes、Select。
...
此选项[`rowGroup.startRender`](https://datatables.net/reference/option/rowGroup.startRender)可以指定一个函数，该函数可以为分组的起始行返回一些复杂数据，如聚合、计数或其他摘要信息。每一次更改页面时如分页、筛选或排序等，都会重新重新调用该函数，使得分组信息保持最新。
...
支持 crosstalk 的交互式绘图包和交互式表格包一起使用时，可以通过 crosstalk 实现图表数据共享，比如 plotly 和 DT。
