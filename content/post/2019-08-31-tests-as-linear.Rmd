...
[^note]: 本文中，factor 翻译为类别变量或因子，level 翻译为取值范围或水平；就译者认为，这两个词语的前一种翻译更贴近常见汉语，后一种翻译虽然简练但是不知所云。---- 译者注

首先创建可能取值为 `a`、`b`、`c` 的类别变量[^note]，那么 **单因素方差分析**基本上成为了**三样本 t 检验**，然后手动对每个组创建[示性变量](#dummy)。
