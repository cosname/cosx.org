...
其中，$x_i$ 是我们熟悉的示性变量。$\beta_0$ 是第一个组在 $\mathrm{age}=0$ 时候的均值。你可以用这个方法，把所有的方差分析转化成为协方差分析，比如说在上一节的**双因素方差分析**加上 $\beta_N \cdot \mathrm{age}$。但我们先继续探究单因素协方差分析，从添加 $\mathrm{age}$ 到模型里开始：
...
比起使用位于 x 轴的位置，最好使用颜色来可视化各组数据。$\beta$ 依然是数据的平均 $y$ 移动量，唯一的不同是我们现在用斜率而不是截距来对每一组进行建模。换句话说，单因素方差分析实际上是对于每一组（$y = \beta_0$）的[单样本 t 检验](#t1)，而**单因素协方差分析**实际上是每一组（$y_i = \beta_0 + \beta_i + \beta_1 \cdot \mathrm{age}$）的[Pearson 相关性检验](#correlation)：
