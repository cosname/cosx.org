...
第一，LMM与JSE-Ridge Regression的关系最为明显。当没有固定效应，$\textbf{Z}$变为单位矩阵的时候，LMM就变为了JSE（这个时候需要$\sigma^2\_e$ 是已知的，不然会有可辨识性的问题。在JSE(James-Stein Estimator)的问题中，$\sigma^2\_e=1$，更详细的描述请参考我的《<a href="/2012/05/chase-after-eb/" target="_blank">那些年我们一起追的EB</a>》）。LMM与Ridge 的关系，前面已经讲过了。
