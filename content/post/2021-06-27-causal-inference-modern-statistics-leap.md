...
`$$
\tau &= \sum_x {E(Y_i(1) | X_i=x) − E(Y_i (0) | X_i = x)}P(X_i = x)\\
&= \sum_x {E(Y_i(1) | Z_i = 1, X_i = x) − E(Y_i (0) | Z_i = 0, X_i = x)}P(X_i = x)\\
&= \sum_x {E(Y_i | Z_i = 1, X_i = x) − E(Y_i | Z_i = 0, X_i = x)}P(X_i = x).
$$`
...
`$$ 
P(X) &= P(X_1) \cdot P(X_2) \cdot P(X_3|X_1,X_2) \cdot P(X_4|X_1) \\
