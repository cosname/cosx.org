...
这就是我教程的第一课。简单！相反，用 Tidy 的群体不使用 base-R 而坚持使用 **ggplot2**绘图（再说一次，其实这并不属于 Tidy，只不过有 Tidy 的倡导者常如此描述）。要想用 Tidy，教员需要如下操作
...
这就是我的观点！**新人应该用循环，而不是purrr**。然而 Tidy 推广者不愿让学生用循环。所以用 Tidy 的教员就得避免给学生用这样的例子，对于同样的例子，用 base-R 的教员就很容易处理。
...
正如我前面所说，与成功使用 Tidyverse 指导初学者程序员的教员讨论时，我问他们学生是否没有能力只学习 base-R，他们承认答案是否定的。事实上，在 Tidyverse 之前，大量在过去没有任何编程背景的人都在学习 base-R，。
