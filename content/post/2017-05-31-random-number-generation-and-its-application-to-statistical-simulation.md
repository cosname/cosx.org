...
先给定阈值delta为0.01（也可以取别的值），在生成的序列中，两个小于delta的值，要间隔很久才出现。根据游程检验的假设，这个的情况要随着间距增大而减少。而通过这个检验，这个序列才能说具有一定的随机性（严谨的说， 应该是不拒绝原序列是随机的）。

代码的实现方式是：取出序列中的值小于delta的位置，位置序列前面添加0，再差分后，然后每个值减1，得到新序列。新序列中的值为0，表明原序列连续两个值小于delta；新序列中的值为1，表明间隔1个数小于delta；新序列中的值为2，表明间隔2个数小于delta，依次类推.....统计所有的情况，用直方图显示（间隔数取到100足可示意）。
