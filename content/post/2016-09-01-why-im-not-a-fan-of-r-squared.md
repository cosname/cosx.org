...
![model_comparison_1](https://cos.name/wp-content/uploads/2016/08/model_comparison_1.png) 在图中，我们可以看到 `$f(x)$` 可以被一条直线很好地近似，所以我们的线性和二次回归模型都可以很好地还原真实模型。这是因为 `$x_{min}$` 和 `$x_{max}$` 非常接近，因此真实的函数在这个区域中可以被直线很好地近似，特别是相对于我们观测的噪声水平而言。
