...
言归正传，如果不把DNN看成上帝/天网/人工智能终点etc.，也暂不考虑当前DL和人脑思维之间若有若无的联系，那么DNN和K-Means、主成分分析（PCA）、稀疏编码（sparse  coding或Lasso）等众多耳熟能详的模型并无二致，都属于机器学习中**特征学习（feature learning）**范畴。假如硬说DNN有什么不同，那么大概就在一个“深”字上。从数据x中学习特征y，如果（绝大多数）传统模型写成y = f(x)（即学习**“一个”**特征变换），DNN则可以写成`$y = f_N(\cdots(f_2(f_1(x))))$` （即学习**“若干个级联”**的特征变换）。那么究竟什么使得DNN如此效果拔群？作者君本人归纳了三点：
