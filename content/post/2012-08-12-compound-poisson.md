...
其中保单到达时刻$M(t)$服从参数为$\lambda\_1$的泊松过程，理赔发生时刻$N(t)$服从参数为$\lambda\_2$的泊松过程，每次支付的保险费$X_i$服从参数为$v$的相互独立的指数分布。

<!--more-->

在这个模型中，保险人期望$cM(t)-\sum\_{i=1}^{N(t)}X\_i$能恒大于0，因此至少$E(cM(t)-\sum\_{i=1}^{N(t)}X\_i)>0$即$c\lambda\_1>\lambda\_2/v$，另外从经验来看，保险事故发生的概率一般不高，一次理赔的的金额应该远大于收到的保费，所以保单到达的速率应该远比理赔发生的速率大，否则这种产品就没有经营价值，保险人也将面临破产，所以$\lambda\_1\gg \lambda\_2$。

考虑以下一个具体的破产模型案例：

某一款保险产品，假设保单到达的速率为$\lambda\_1=10$张/天，理赔发生的速率为$\lambda\_2=1$次/天。假设每张保单价格c=120，理赔额服从参数为$v=1/1000$（以$c\lambda\_1=1.2\lambda\_2/v$设定）的指数分布。设定初始$u=3000$时，计算到第1000天为止发生破产的概率。

本文用R语言模拟了10000次，用时1625秒，大概不到半个小时的时间，时间还能接受。最终结果10000次中破产5293次，破产率大概53%。输出各阶段破产时刻频数和频率结果如下：
  
[<img class="aligncenter size-full wp-image-6197" title="sim1" src="http://cos.name/wp-content/uploads/2012/08/sim1-e1344696345207.png" alt="" width="490" height="247" srcset="http://cos.name/wp-content/uploads/2012/08/sim1-e1344696345207.png 490w, http://cos.name/wp-content/uploads/2012/08/sim1-e1344696345207-300x151.png 300w" sizes="(max-width: 490px) 100vw, 490px" />](http://cos.name/wp-content/uploads/2012/08/sim1.png)

直方图为：
  
[<img class="aligncenter size-large wp-image-6184" title="hist1" src="http://cos.name/wp-content/uploads/2012/08/hist1-500x500.png" alt="" width="500" height="500" srcset="http://cos.name/wp-content/uploads/2012/08/hist1-500x500.png 500w, http://cos.name/wp-content/uploads/2012/08/hist1-150x150.png 150w, http://cos.name/wp-content/uploads/2012/08/hist1-300x300.png 300w, http://cos.name/wp-content/uploads/2012/08/hist1.png 672w" sizes="(max-width: 500px) 100vw, 500px" />](http://cos.name/wp-content/uploads/2012/08/hist1.png)
...
$$U(t) = u + \sum^{N(t)}\_{i = 1}c\_iM\_i(t) &#8211; \sum^{N\_i(t)}\_{j = 1}\sum^{n}\_{i = 1}X\_{ij}, \quad t \geq 0; u \geq 0; c\_i > 0$$

将模型展开，可以表示为：

$$U(t) = u + \sum^{N(t)}\_{i = 1}c\_iM\_i(t) &#8211; (\sum^{N\_1(t)}\_{j = 1}X\_{1j} + \sum^{N\_2(t)}\_{j = 1}X\_{2j} + \cdots + \sum^{N\_n(t)}\_{j = 1}X\_{nj})$$

这个模型考虑有n款不同的保险产品，保费分别用$c\_i$表示，第i款产品的保单到达时刻为$M\_i(t)$。这样需要对应n款产品的理赔，假设第i款产品的理赔到达时刻为$N\_i(t)$，对应的理赔额为$X\_{ij}$，表示第i款产品第j次的理赔额。根据上一个模型的情况，这里假设$M\_i(t)$是服从参数为$\alpha\_i$的泊松过程，$N\_i(t)$是服从参数为$\beta\_i$的泊松过程，$X\_{ij}$是服从参数为$v\_i$的指数分布。同样地，根据每个保单的对应情况，要求满足$c\_i \alpha\_i > \beta\_i / v\_i$和$\alpha\_i > \beta\_i$。

考虑以下一个具体的破产模型案例：
  
某保险人同时经营了三款不同的保险产品，假设保单到达的速率为$\alpha\_i = 10$每一单位时间（简化起见，不考虑实际单位，改用形式的一单位时间），$\alpha\_i = 20$，$\alpha\_i = 30$。理赔发生的速率为$\beta\_i = 1$，$\beta\_i = 2$，$\beta\_i = 3$。$X\_{1j}$服从参数$v\_1 = 1/1000$的指数分布，假定$v\_2 = 1/1500$，$v\_3 = 1/2000$。以（20%预期收益率）的标准计算$c\_i$的值。$c\_1 = 120$，$c\_2 = 180$，$c\_3 = 240$。
...
[<img class="aligncenter size-large wp-image-6193" title="sim2" src="http://cos.name/wp-content/uploads/2012/08/sim2-500x239.png" alt="" width="500" height="239" srcset="http://cos.name/wp-content/uploads/2012/08/sim2-500x239.png 500w, http://cos.name/wp-content/uploads/2012/08/sim2-300x143.png 300w, http://cos.name/wp-content/uploads/2012/08/sim2.png 524w" sizes="(max-width: 500px) 100vw, 500px" />](http://cos.name/wp-content/uploads/2012/08/sim2.png)

破产时刻分布的直方图如下：
  
[<img class="aligncenter size-large wp-image-6186" title="hist2" src="http://cos.name/wp-content/uploads/2012/08/hist2-500x500.png" alt="" width="500" height="500" srcset="http://cos.name/wp-content/uploads/2012/08/hist2-500x500.png 500w, http://cos.name/wp-content/uploads/2012/08/hist2-150x150.png 150w, http://cos.name/wp-content/uploads/2012/08/hist2-300x300.png 300w, http://cos.name/wp-content/uploads/2012/08/hist2.png 672w" sizes="(max-width: 500px) 100vw, 500px" />](http://cos.name/wp-content/uploads/2012/08/hist2.png)
...
[<img class="aligncenter size-large wp-image-6187" title="hist3" src="http://cos.name/wp-content/uploads/2012/08/hist3-500x286.png" alt="" width="500" height="286" srcset="http://cos.name/wp-content/uploads/2012/08/hist3-500x286.png 500w, http://cos.name/wp-content/uploads/2012/08/hist3-300x171.png 300w, http://cos.name/wp-content/uploads/2012/08/hist3.png 867w" sizes="(max-width: 500px) 100vw, 500px" />](http://cos.name/wp-content/uploads/2012/08/hist3.png)
...
> 本文完整版（含源代码）请**[点击此处](http://cos.name/wp-content/uploads/2012/08/复合泊松破产模型的推广和R语言环境下的随机模拟.pdf)**。
