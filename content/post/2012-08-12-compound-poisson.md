...
其中保单到达时刻`$M(t)$`服从参数为`$\lambda_1$`的泊松过程，理赔发生时刻`$N(t)$`服从参数为`$\lambda\_2$`的泊松过程，每次支付的保险费`$X_i$`服从参数为`$v$`的相互独立的指数分布。
...
![sim1](https://cos.name/wp-content/uploads/2012/08/sim1-e1344696345207.png)
...
`$$U(t) = u + \sum^{N(t)}\_{i = 1}c_iM_i(t) – \sum^{N_i(t)}_{j = 1}\sum^{n}_{i = 1}X_{ij}, \quad t \geq 0; u \geq 0; c\_i > 0$$`
