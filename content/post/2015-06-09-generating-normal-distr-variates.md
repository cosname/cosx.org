...
之前拜读了靳志辉（<a href="http://www.weibo.com/rickjin" target="_blank">@rickjin</a>）老师写的<a href="http://cos.name/2013/01/story-of-normal-distribution-1/" target="_blank">《正态分布的前世今生》</a>，一直对正态分布怀着一颗敬畏之心，刚好最近偶然看到`python`标准库中如何生成服从正态分布随机数的源码，觉得非常有趣，于是又去查找其他一些生成正态分布的方法，与大家分享一下。
...
[<img class="aligncenter size-full wp-image-11054" src="http://cos.name/wp-content/uploads/2015/06/generate-normal-1.png" alt="generate-normal-1" width="422" height="374" srcset="http://cos.name/wp-content/uploads/2015/06/generate-normal-1.png 422w, http://cos.name/wp-content/uploads/2015/06/generate-normal-1-300x266.png 300w" sizes="(max-width: 422px) 100vw, 422px" />](http://cos.name/wp-content/uploads/2015/06/generate-normal-1.png)可以看到，`n=1`时其实就是均匀分布，随着`n`逐渐增大，直方图轮廓越来越接近正态分布了~因此利用中心极限定理暴力生成服从正态分布的随机数是可行的。但是这样生成正态分布速度是非常慢的，因为要生成若干个同分布随机变量，然后求和、计算，效率是非常低的。
...
[<img class="aligncenter size-full wp-image-11055" src="http://cos.name/wp-content/uploads/2015/06/generate-normal-2.png" alt="generate-normal-2" width="350" height="224" srcset="http://cos.name/wp-content/uploads/2015/06/generate-normal-2.png 350w, http://cos.name/wp-content/uploads/2015/06/generate-normal-2-300x192.png 300w" sizes="(max-width: 350px) 100vw, 350px" />](http://cos.name/wp-content/uploads/2015/06/generate-normal-2.png)在`y`轴上产生服从(0,1)均匀分布的随机数，水平向右投影到曲线上，然后垂直向下投影到`x`轴，这样在`x`轴上就得到了正态分布。
...
[<img class="aligncenter size-large wp-image-11056" src="http://cos.name/wp-content/uploads/2015/06/generate-normal-3-500x377.png" alt="generate-normal-3" width="500" height="377" srcset="http://cos.name/wp-content/uploads/2015/06/generate-normal-3-500x377.png 500w, http://cos.name/wp-content/uploads/2015/06/generate-normal-3-300x226.png 300w, http://cos.name/wp-content/uploads/2015/06/generate-normal-3.png 815w" sizes="(max-width: 500px) 100vw, 500px" />](http://cos.name/wp-content/uploads/2015/06/generate-normal-3.png)以上两个方法虽然方便也容易理解，但是效率实在太低，并不实用，那么在实际中到底是如何生成正态分布的呢？
...
[<img class="aligncenter size-full wp-image-11057" src="http://cos.name/wp-content/uploads/2015/06/generate-normal-4.png" alt="generate-normal-4" width="352" height="267" srcset="http://cos.name/wp-content/uploads/2015/06/generate-normal-4.png 352w, http://cos.name/wp-content/uploads/2015/06/generate-normal-4-300x228.png 300w" sizes="(max-width: 352px) 100vw, 352px" />](http://cos.name/wp-content/uploads/2015/06/generate-normal-4.png)这里抽样次数达到1千万次，1秒左右就完成了，速度非常快~
...
[<img class="aligncenter size-large wp-image-11058" src="http://cos.name/wp-content/uploads/2015/06/generate-normal-5-500x257.png" alt="generate-normal-5" width="500" height="257" srcset="http://cos.name/wp-content/uploads/2015/06/generate-normal-5-500x257.png 500w, http://cos.name/wp-content/uploads/2015/06/generate-normal-5-300x154.png 300w, http://cos.name/wp-content/uploads/2015/06/generate-normal-5.png 566w" sizes="(max-width: 500px) 100vw, 500px" />](http://cos.name/wp-content/uploads/2015/06/generate-normal-5.png)具体操作如下，设定一个方便抽样的函数$q(x)$，以及一个常量$k$，使得$p(x)$总在$kq(x)$的下方。（参考上图）
...
[<img class="aligncenter size-full wp-image-11059" src="http://cos.name/wp-content/uploads/2015/06/generate-normal-6.png" alt="generate-normal-6" width="436" height="248" srcset="http://cos.name/wp-content/uploads/2015/06/generate-normal-6.png 436w, http://cos.name/wp-content/uploads/2015/06/generate-normal-6-300x171.png 300w" sizes="(max-width: 436px) 100vw, 436px" />](http://cos.name/wp-content/uploads/2015/06/generate-normal-6.png)而Ziggurat算法高效的秘密在于构造了一个非常精妙的$q(x)$，看下面这张图

[<img class="aligncenter size-large wp-image-11060" src="http://cos.name/wp-content/uploads/2015/06/generate-normal-7-461x500.png" alt="generate-normal-7" width="461" height="500" srcset="http://cos.name/wp-content/uploads/2015/06/generate-normal-7-461x500.png 461w, http://cos.name/wp-content/uploads/2015/06/generate-normal-7-277x300.png 277w, http://cos.name/wp-content/uploads/2015/06/generate-normal-7.png 737w" sizes="(max-width: 461px) 100vw, 461px" />](http://cos.name/wp-content/uploads/2015/06/generate-normal-7.png)我们用多个堆叠在一起的矩形，这样保证阴影部分（被拒绝部分）的始终较小，这样就非常高效了
...
### [<img class="aligncenter size-large wp-image-11061" src="http://cos.name/wp-content/uploads/2015/06/generate-normal-8-500x376.png" alt="generate-normal-8" width="500" height="376" srcset="http://cos.name/wp-content/uploads/2015/06/generate-normal-8-500x376.png 500w, http://cos.name/wp-content/uploads/2015/06/generate-normal-8-300x226.png 300w, http://cos.name/wp-content/uploads/2015/06/generate-normal-8.png 512w" sizes="(max-width: 500px) 100vw, 500px" />](http://cos.name/wp-content/uploads/2015/06/generate-normal-8.png)总结
