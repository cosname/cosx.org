...
description: "**echarts4r** 是 R 中的一个画图工具包，作者是 John Coene。这个包的名字读起来是 Echarts for R，顾名思义就是在 R 中应用 Apache Echarts。Apache ECharts 是一个基于 JavaAcript 的开源可视化图表库，官网有中英两种语言的文档，对英文阅读水平弱一些的小伙伴比较友好。"
forum_id: 
...
不难发现，**echarts4r.maps** 内置的中国地图是不完整的，缺少南海诸岛及九段线等内容，因此，需要获取完整正确的地图，高德和百度都是具有甲级测绘资质的单位。下面采用[阿里 DataV 数据可视化平台](https://datav.aliyun.com/portal/school/atlas/area_selector)开放出来的地图，数据集保存到本地，实际上，原始地图数据来自[高德开放平台](https://lbs.amap.com/api/webservice/guide/api/district)。地图数据包含行政唯一编码、行政单位名称和行政级别（省、市和县三级），在将数据映射到背景地图的对应区域时，需要保证数据集中的行政单位名称和 背景地图中的行政单位名称是一一对应的。
