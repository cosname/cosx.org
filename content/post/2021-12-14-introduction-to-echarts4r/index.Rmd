...
[echarts4r](https://echarts4r.john-coene.com/index.html) 是 R 中的一个画图工具包，作者是 John Coene。这个包的名字读起来是 Echarts for R，顾名思义就是在 R 中应用 [Apache Echarts](https://echarts.apache.org/zh/index.html)。Apache ECharts 是一个基于 JavaAcript 的开源可视化图表库，官网有中英两种语言的文档，对英文阅读水平弱一些的小伙伴比较友好。
...
>注1：本文所使用 R 版本为4.1.1 ，echarts4r 包的版本为0.4.2 ，对应 Echarts 版本为5.0以上，R 4.1.0 之前的版本应将所有的`|>`换成`%>%` 。

>注2：一个标准数据框里的一列数据在不同领域里有不同的名字，如字段、数据项、变量、变量、特征、系列等等，本文统称为变量。
...
echarts4r包官网提供了[很丰富的主题样式](https://echarts4r.john-coene.com/articles/chart_types.html)，不喜欢默认主题的话，可以换其他的主题。
